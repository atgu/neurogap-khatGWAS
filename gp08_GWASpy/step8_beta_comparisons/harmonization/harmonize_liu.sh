...
cat > /tmp/harmonize_liu_chunked.py << 'EOF'
import pandas as pd
from pathlib import Path

liu_dir = Path("~/sud/liu-2019/liftover_only").expanduser()
khat_dir = Path("~/sud/khat").expanduser()
out_dir = Path("~/rsid_match/liu_lifted_harmonized").expanduser()
...
liu_files = [
    "AgeOfInitiation.WithoutUKB.txt.lifted_hg38.tsv",
    "AgeofInitiation.txt.lifted_hg38.tsv",
    "CigarettesPerDay.WithoutUKB.txt.lifted_hg38.tsv",
    "CigarettesPerDay.txt.lifted_hg38.tsv",
    "DrinksPerWeek.WithoutUKB.txt.lifted_hg38.tsv",
    "DrinksPerWeek.txt.lifted_hg38.tsv",
    "SmokingCessation.WithoutUKB.txt.lifted_hg38.tsv",
    "SmokingCessation.txt.lifted_hg38.tsv",
    "SmokingInitiation.WithoutUKB.txt.lifted_hg38.tsv",
    "SmokingInitiation.txt.lifted_hg38.tsv"
]
...
def harmonize_chunk(khat, liu, trait, khat_beta_col, khat_p_col):
    merged = khat.merge(liu, on=["CHR", "BP"], how="inner")

    if len(merged) == 0:
        return merged

    merged["liu_A1_comp"] = merged["liu_A1"].apply(complement_allele)
    merged["liu_A2_comp"] = merged["liu_A2"].apply(complement_allele)

    exact_match = (
        (merged["khat_A1"] == merged["liu_A1"]) &
        (merged["khat_A2"] == merged["liu_A2"])
    )

    swapped_match = (
        (merged["khat_A1"] == merged["liu_A2"]) &
        (merged["khat_A2"] == merged["liu_A1"])
    )

    complement_match = (
        (merged["khat_A1"] == merged["liu_A1_comp"]) &
        (merged["khat_A2"] == merged["liu_A2_comp"])
    )

    complement_swapped_match = (
        (merged["khat_A1"] == merged["liu_A2_comp"]) &
        (merged["khat_A2"] == merged["liu_A1_comp"])
    )
    ...
    keep_cols = [
        ...
        "liu_marker", "liu_A1", "liu_A2",
        beta_raw_col, beta_harmonized_col,
        "PVALUE", "SE", "N", "EFFECTIVE_N",
        "allele_match_status"
    ]

    for col in ["AF", "STAT", "Number_of_Studies", "ANNO", "ANNOFULL"]:
...
for filename in liu_files:
    trait = filename.replace(".txt.lifted_hg38.tsv", "").replace(".WithoutUKB", "_WithoutUKB")
    ...
    for chunk_i, df in enumerate(reader):
        ...
        df["liu_marker"] = df["RSID"].astype(str)
        df["liu_A1"] = df["REF"].astype(str).str.upper()
        df["liu_A2"] = df["ALT"].astype(str).str.upper()

        df = df[
            df["liu_A1"].apply(is_snv) &
            df["liu_A2"].apply(is_snv)
        ].copy()

        df = df[
            ~df.apply(lambda r: is_ambiguous(r["liu_A1"], r["liu_A2"]), axis=1)
        ].copy()

        df = df.drop_duplicates(subset=["CHR", "BP", "liu_A1", "liu_A2"])
...
python3 /tmp/harmonize_liu_chunked.py
