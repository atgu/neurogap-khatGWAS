...
SITE <- "Moi"   # or "Uganda", "Moi", "KEMRI", "ALL"
# no UCT, not enough non-users
...
plink_dir <- "/Users/nvalenci/Desktop/work/khat_gwas/script_referenced_files/gwaspy_ldpruned"
pheno_dir <- "/Users/nvalenci/Desktop/work/khat_gwas/phenotype_files/StudySite_w_MatchingIIDs"
out_dir   <- "/Users/nvalenci/Desktop/work/khat_gwas/plink_by_site_null/Nas0"
...
# Phenotype files
pheno_paths <- list(
  AAU    = file.path(pheno_dir, "no_char_AAU_khat_pheno_subset_lerato_with_studysite.tsv"),
  Uganda = file.path(pheno_dir, "no_char_Uganda_khat_pheno_subset_lerato_with_studysite.tsv"),
  Moi    = file.path(pheno_dir, "no_char_Moi_khat_pheno_subset_lerato_with_studysite.tsv"),
  KEMRI  = file.path(pheno_dir, "no_char_KEMRI_khat_pheno_subset_lerato_with_studysite.tsv"),
  UCT    = file.path(pheno_dir, "no_char_UCT_khat_pheno_subset_lerato_with_studysite.tsv")
)
...
covars_map <- list(
  ...
  Uganda = list(covars = c("age","sex","PC1","PC2","PC3","PC4","PC5","study_site"),
                categoricals = c("sex","study_site")),
  Moi    = list(covars = c("age","sex","PC1","PC2","PC3","PC4","PC5","study_site"),
                categoricals = c("sex","study_site")),
  KEMRI  = list(covars = c("age","sex","PC1","PC2","PC3","PC4","PC5","study_site"),
                categoricals = c("sex","study_site"))
)
...
run_site_null <- function(site, threads = 4) {
  ...
  subjData_vec <- keep$IID
   # Define simple output name
  out_prefix <- paste0(site, "_kufNAs0_w_site_polmm_dense")
  ...
  objNull <- POLMM_Null_Model(
    ...
    control   = list(threads = threads)
  )

  # Save null
  null_path <- file.path(out_dir, paste0(out_prefix, ".rds"))
